...
A FastAPI gateway that proxies the GeoSWMM AI add-in to OpenRouter and a local pgvector store for embeddings/RAG. The trust boundary lives here: server-side keys never reach the add-in.
...
- **pgvector** extension for the local vector backend serving `/v1/vectors/*`
...
├── routes/            # messages.py, embeddings.py, vectors.py, usage.py
├── upstream/          # openrouter.py, pgvector.py — async wrappers
...
### Secrets
- Server keys (OpenRouter, JWT secret, Postgres password) live only in `.env` on the VPS.
...
- `/v1/vectors/*` tests run against the real pgvector test DB. Seed and clean up rows explicitly per test (see `test_vectors.py`).
...
# fill in OPENROUTER_API_KEY, generate JWT_SECRET, POSTGRES_PASSWORD
...
- `/v1/vectors/search` and `/v1/vectors/upsert` are served by `gateway.upstream.pgvector` against the local Postgres. `request_log.endpoint` is `"vectors.search"` / `"vectors.upsert"`; `cost_usd` is always `0.000000` (no per-call billing).
- The filter DSL covers only the `must` / `must_not` / `match` / `range` shapes used by the add-in. Anything outside that subset returns `400 {"error": "invalid_filter"}`. Extend the DSL deliberately.
...
5. **P5** — `/v1/embeddings` and `/v1/vectors/*` proxy and log
