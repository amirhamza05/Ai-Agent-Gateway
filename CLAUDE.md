...
A FastAPI gateway that proxies the GeoSWMM AI add-in to OpenRouter and Qdrant. The trust boundary lives here: server-side keys never reach the add-in.
...
- **PostgreSQL 16** via **SQLAlchemy 2.0 async** + **asyncpg** driver
...
├── upstream/          # openrouter.py, qdrant.py — async httpx wrappers
